<script lang="ts">
    ...
    function handleMouseEnter(ref: BibleRef, word_num: number) {
        ...
        instant_details_timeout = setTimeout(() => {
            EventsEmit("window:workspace:instantdetails", ref, word_num);
        }, INSTANT_DETAILS_DELAY);
    }

    function handleMouseLeave() {
        clearTimeout(instant_details_timeout);
        EventsEmit("window:workspace:instantdetails:hide");
    }
</script>
...
<div class="block">
    ...
    {#if !("continuation" in block.verses[0])}
        <span class="ref">
            {refToString($workspace_version, block.range.start, "short")}
        </span>
    {/if}
