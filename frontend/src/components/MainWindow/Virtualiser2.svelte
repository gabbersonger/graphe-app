<script lang="ts">
    ...
    type VirtualRowLockData = {
        total_rows_height: number;
        scroll: number;
        block: number;
        block_offset: number;
    };
    let lock_data: VirtualRowLockData = undefined;
    ...
    function resize() {
        ...
        if (mode == "precise") {
            mode = "lock";
            lock_data = {
                total_rows_height: total_rows_height,
    ...
    async function preloadBatch() {
        ...
        for (let i = 0; i < preloading.length; i++) {
            const preload_block = n_preloaded + i;
            if (!row_elements[preload_block]) return;
            row_offsets[preload_block] = total_rows_height;
            total_rows_height += row_elements[preload_block].offsetHeight;

            if (mode == "lock" && lock_data.block == preload_block) unlock();
        }
    ...
    function unlock() {
        viewport.scrollTop =
            row_offsets[lock_data.block] +
            (viewport.scrollTop - lock_data.block_offset);
        mode = "precise";
        lock_data = null;
    }
            ...
            if (isRefInRange(ref, section.range)) {
                for (let j = 0; j < section.blocks.length; j++) {
                    ...
                    if (isRefInRange(ref, block.range)) {
                        ...
                        viewport.scrollTop = scrollPosition + current_offset;
                        return;
                    }
                    n_seen += 1;
                }
...
<div class="container">
    <div class="viewport" bind:this={viewport} on:scroll={refresh}>
        <div class="content" bind:this={content}>
            <div
                class="rows"
                style:min-height={mode == "precise"
                    ? `${total_rows_height}px`
                    : `${lock_data.total_rows_height}px`}
            >
                {#if mode == "lock"}
                    <!-- TODO: add all the items prior to block to a floating window above the lock offset -->
                    <div>items prior to lock block</div>
                {/if}
